...
with open('day12.txt') as file:
    for line in file:
        ...
        brokenidx=pattern.find('#') if pattern.find('#')!=-1 else float('inf')
        questionidx=pattern.find('?') if pattern.find('?')!=-1 else float('inf')
        if brokenidx==questionidx==float('inf') or questionidx==float('inf'):
            ans+=1
        else:
            ans+=dfs(pattern[min(brokenidx,questionidx):],counts)
print(ans)
